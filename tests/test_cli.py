...
def test_delete_trash_fails_before_connect(store, fake, capsys):
    fake["factory"] = lambda addr: (_ for _ in ()).throw(AssertionError("must not connect"))
    rc, out, err = run(capsys, "delete", "--trash", str(SID), "1")
    assert rc == cli.EXIT_USAGE
    assert "unsupported" in err


def test_sync_moments_requires_session(capsys):
    rc, out, err = run(capsys, "sync", "--moments", "1,2")
    assert rc == cli.EXIT_USAGE
    assert "--session" in err


def test_bad_moment_ids_are_usage_error(capsys):
    with pytest.raises(SystemExit) as ei:
        cli.main(["encode", "--session", "1", "--moments", "nope"])
    assert ei.value.code == 2


def test_capture_wait_nonzero_if_not_capture(store, fake, capsys):
    key = b"\x0e" * 32
    s = PairingStore(store)
    s.put(Pairing("AA:BB:CC:DD:EE:FF", key))
    s.save()
    fake["factory"] = lambda addr: FakeLens(pairing_key=key)
    rc, out, err = run(capsys, "capture", "--wait", "1")
    assert rc == cli.EXIT_CAMERA
    assert "did not enter CAPTURE" in out


def test_watch_json_is_one_object(store, fake, capsys):
    key = b"\x0f" * 32
    s = PairingStore(store)
    s.put(Pairing("AA:BB:CC:DD:EE:FF", key))
    s.save()
    fake["factory"] = lambda addr: FakeLens(pairing_key=key)
    rc, out, err = run(capsys, "--json", "watch", "--duration", "0.05")
    assert rc == 0
    first = json.loads(out.strip().splitlines()[0])
    assert first["event"] == "initial" and "state" in first
